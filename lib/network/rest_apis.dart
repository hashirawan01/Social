...
Future<List<ProfileFieldModel>> getProfileFields({int signUpOnly = 0}) async {
  Iterable it = await handleResponse(
    await buildHttpResponse(
      '${APIEndPoint.getProfileFields}?sign_up_only=$signUpOnly${signUpOnly == 0 ? '&user_id=${appStore.loginUserId}' : ''}',
      method: HttpMethod.GET,
      passHeaders: signUpOnly == 0,
    ),
  );
...
Future<void> uploadStory(
  ...
  Future.forEach<MediaSourceModel>(fileList, (element) async {
    ...
    multiPartRequest.files.forEach((element) {
      log('files ${element.field} :${element.filename} ${element.contentType}');
    });
...
Future<List<ProductListModel>> getProductsList({int page = 1, int? categoryId, String? orderBy, String? searchText}) async {
  ...
  if (categoryId != null) {
    ...
    it = await handleResponse(await buildHttpResponse('${APIEndPoint.productsList}?orderby=$orderBy${searchText.validate().isNotEmpty ? '&search=$searchText' : ''}&page=$page&per_page=$PER_PAGE',
        passParameters: true, passToken: false));
  }
