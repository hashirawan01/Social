...
class _ChatScreenPopUpMenuComponentState extends State<ChatScreenPopUpMenuComponent> {
  Future<void> onStartCall({required String callType}) async {
    ...
    if (appStore.isWebsocketEnable == 1) {
      String message =
          '42["${SocketEvents.mediaEvent}",{"event":"${SocketEvents.calling}","to":${widget.user!.userId.validate()},"thread_id":${widget.threadId},"type":"$callType","avatar":"${appStore.loginAvatarUrl}","name":"${appStore.loginFullName}"}]';
    ...
    await startCall(request: request).then((value) async {
      if (await Permissions.cameraAndMicrophonePermissionsGranted()) {
        UserModel sender = UserModel(
          ...
          photoUrl: appStore.loginAvatarUrl,
          uid: appStore.loginUserId,
          oneSignalPlayerId: [getStringAsync(SharePreferencesKey.ONE_SIGNAL_PLAYER_ID)],
        );
