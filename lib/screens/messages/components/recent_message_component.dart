  ...
  String messagesTitle() {
    if (thread.participantsCount.validate() > 2) {
      ...
      if (thread.subject.validate().isNotEmpty)
        ...
        return '${thread.participantsCount} ${language.participants}';
    } else if (thread.participantsCount.validate() <= 1) {
      return language.deletedUser;
    } else {
                                ...
                                message.message.validate() == MessageText.onlyFiles
                                    ...
                                    : user.blocked == 0 || user.blocked == null
                                        ? message.message.validate().contains('href')
                                            ? language.link
                                            : parseHtmlString(message.message.validateAndFilter())
              ...
              onSelected: (val) async {
                if (val == 1) {
                  if (thread.isPinned == 1) {
                    thread.isPinned = 0;
                    await unPinThread(threadId: thread.threadId.validate()).catchError(onError);
                  } else {
                    thread.isPinned = 1;
                    await pinThread(threadId: thread.threadId.validate()).catchError(onError);
                  }
                  refreshThread.call();
                } else if (val == 2) {
                  ...
                  if (!thread.permissions!.isMuted.validate()) {
                    await unMuteThread(threadId: thread.threadId.validate()).catchError(onError);
                  } else {
                    await muteThread(threadId: thread.threadId.validate()).catchError(onError);
                  }
                } else {
            ...
            if (messageStore.unreadThreads.any((element) => element.threadId == thread.threadId)) {
              ...
              return Positioned(
                ...
                child: Container(
                  ...
                  width: 16,
                  decoration: BoxDecoration(color: Colors.amber, shape: BoxShape.circle),
                  child: FittedBox(child: Text(thread.unread.toString(), style: secondaryTextStyle(color: Colors.white))),
                ),
              );
            } else {
              return Offstage();
