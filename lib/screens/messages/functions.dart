...
import 'package:socialv/screens/messages/calls/media_event_model.dart';
...
void handleSocketEvents(BuildContext context, String message) {
  ...
  } else if (event == SocketEvents.mediaEvent) {
    log('------------ $event');

    String? object = getStreamMessage(message);
    MediaEventModel streamMessage = MediaEventModel.fromJson(jsonDecode(object!));
  }
}
...
void callStream(BuildContext context) {
  callService.callStream(uid: appStore.loginUserId).listen((event) {
    if (event.data() != null) {
